...
export const getEventTypeId = async (eventSlug: string): Promise<string | null> => {
  try {
    ...
    // 🔍 Log para ver quais eventos existem
    console.log("📋 Lista de eventos disponíveis:");
    events.forEach((e: any) => console.log(`- Slug: ${e.slug}, Nome: ${e.name}`));

    // 🔥 Busca insensível a maiúsculas/minúsculas
    const event = events.find((e: any) => e.slug.toLowerCase() === eventSlug.toLowerCase());

    if (!event) {
      console.error(`❌ Evento não encontrado! Slug recebido: "${eventSlug}". Verifique a lista de eventos acima.`);
      return null;
    }
...
export const getAvailableTimes = async (daysAhead: number = 30): Promise<any[]> => {
  try {
    // Busca todos os tipos de evento e seleciona o primeiro disponível
    const events = await getEventTypes();
    if (!events || events.length === 0) throw new Error("Nenhum evento encontrado!");

    const eventType = events[0];
    const eventId = eventType.uri; // Assumindo que a propriedade "uri" é o ID do evento
...
/** 
 * 🔥 Criar um **link de agendamento de uso único** automaticamente
 */
export async function createSchedulingLink() {
  try {
    const events = await getEventTypes(); // 🔥 Obtém todos os eventos disponíveis

    if (!events || events.length === 0) {
      throw new Error("❌ Nenhum evento disponível para criar link de agendamento.");
    }

    const eventType = events[0]; // 🔥 Seleciona o primeiro evento disponível automaticamente
    console.log("✅ Evento selecionado automaticamente:", eventType.name, "-", eventType.uri);

    const requestBody = {
      owner: eventType.uri, // 🔥 Usa o ID do evento automaticamente
      owner_type: "EventType",
      max_event_count: 1, // 🔥 O link expira após um único agendamento
    };
    ...
    console.error("❌ Erro ao criar link de agendamento:", JSON.stringify(error.response?.data || error.message, null, 2));
    return null;
  }
}
